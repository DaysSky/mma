...
public class RuinStateTracker implements StateTracker {
    ...
    public void onActionBar(Component message) {
        if (MMAClient.features().enableTimerAndStats) {
            ...
            if (raw.contains("Masked Killed")) {
                ...
                if (this.data.soulCount >= 450 && this.data.soulTime == -1) {
                    this.data.soulTime = this.logTime("souls", MMAClient.config().ruin.soulsSplit, this.startTime, this.startTime, Util.now());
                }
    ...
    public List<Component> getAdditionalSidebarText() {
        ...
        parts.add(Component.translatable("hud.mma.sidebar.timer", new Object[]{FormatUtil.timestamp(Util.now() - this.startTime)}));
        parts.add(Component.translatable("hud.mma.sidebar.ruin.chests", new Object[]{FormatUtil.numeric(this.data.chestCount)}));
        parts.add(Component.translatable("hud.mma.sidebar.ruin.souls", new Object[]{FormatUtil.numeric(this.data.soulCount)}));
        return parts;
    }
