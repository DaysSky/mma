...
import com.dayssky.mma.features.cz.CharmEffect;
...
public final class Charm {
    ...
    private boolean selfTest(List<String> rawLore) {
        ...
        if (computedBudget != this.maxBudget) {
            hasWarning = true;
            MMAClient.LOGGER.error("Budget algo is likely bugged: dayssky={}, monumenta={}", computedBudget, this.maxBudget);
        }
        ...
        for (CharmEffect effect : this.effects) {
            ...
            if (!rawLore.contains(text)) {
                hasWarning = true;
                MMAClient.LOGGER.error("not found: {}", text);
            }
        }
