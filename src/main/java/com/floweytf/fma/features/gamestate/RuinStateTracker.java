...
public class RuinStateTracker implements StateTracker {
        ...
        if (FMAClient.features().enableTimerAndStats) {
            ...
            if (raw.contains("Masked Killed")) {
                ...
                if (this.data.soulCount >= 450 && this.data.soulTime == -1) {
                    this.data.soulTime = this.logTime("souls", FMAClient.config().ruin.soulsSplit, this.startTime, this.startTime, Util.now());
                }
    ...
    public List<Component> getAdditionalSidebarText() {
        ...
        parts.add(Component.translatable("hud.fma.sidebar.ruin.chests", new Object[]{FormatUtil.numeric(this.data.chestCount)}));
        parts.add(Component.translatable("hud.fma.sidebar.ruin.souls", new Object[]{FormatUtil.numeric(this.data.soulCount)}));
        return parts;
    }
