...
public class VersionChecker {
    ...
    private static final HttpRequest REQUEST = HttpRequest.newBuilder()
            .uri(URI.create("https://api.github.com/repos/Floweynt/flowey-monumenta-addons/releases"))
            .GET()
            .build();
        ...
        ClientJoinServerEvent.EVENT
                .register(
                        (ClientJoinServerEvent) () -> {
                            ...
                            switch (info.state) {
                                case NOT_READY:
                                    ChatUtil.sendWarn(Component.translatable("text.fma.version.common.update_check_timeout"));
                                    break;
                                case NOT_AVAILABLE:
                                    ChatUtil.sendWarn(Component.translatable("text.fma.version.common.update_check_fail"));
                                case DISABLED:
                                default:
                                    break;
                                case OUTDATED:
                                    ChatUtil.send(
                                            Component.translatable("text.fma.version.common.new_version", new Object[]{FormatUtil.altText(info.unwrap().getFriendlyString())})
                                    );
                            }
                        }
                );
        ...
        if (this.latestVersion.isDone()) {
            if (this.latestVersion.isCompletedExceptionally()) {
                ...
                if (res.isEmpty()) {
                    ...
                    int comp = res.get().compareTo(FMAClient.MOD.getMetadata().getVersion());
                    if (comp == 0) {
